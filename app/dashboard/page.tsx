...
const WELCOME_BONUS = 180_000 // ₦180,000
...
export default function DashboardPage() {
  ...
  const [currentLevel, setCurrentLevel] = useState<string | null>(null) // New state for current level
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (walletErr) {
        // You may want to log or toast this error. We'll just keep defaults.
      }

      if (!walletRow) {
        // Create wallet row with welcome bonus
      ...
      // 2) Optionally detect if user already has Pay ID (quick check)
      let hasPayId = false
      {
      ...
      try {
        // Get profile with upgrade_level_id
        ...
        if (profileError || !profileData?.upgrade_level_id) {
        ...
        setCurrentLevel(levelError ? null : levelData?.name || null);
      } catch (error) {
        console.error("Error fetching current level:", error);
        setCurrentLevel(null);
      }

      // 4) Popup logic (unchanged)
      const welcomePopupShown = localStorage.getItem("paygo-welcome-popup-shown")
      if (!welcomePopupShown) {
        setShowWelcomePopup(true)
      } else {
        const lastNewMonthPopup = localStorage.getItem(
        ...
        if (lastNewMonthPopup !== currentMonthKey) {
          ...
          const lastFastPaymentPopup = localStorage.getItem(
  ...
  return (
    <div className="min-h-screen pb-4 bg-gradient-to-b from-[#f9f2f2] to-[#f5f0ff]">
      ...
      {showFastPaymentPopup && (
        <FastPaymentPopup onClose={handleCloseFastPaymentPopup} />
      )}
      ...
      <div className="bg-white text-green-600 text-xs py-3 px-4 shadow-sm w-full">
        <div className="overflow-hidden whitespace-nowrap">
          <div className="animate-marquee inline-block font-bold">
            🚀 Great News! Payment processing is now lightning fast! Make your
            payments and get verified instantly! &nbsp;&nbsp;&nbsp; 🚀 Great
            News! Payment processing is now lightning fast! Make your payments
            and get verified instantly!
          </div>
        </div>
      </div>
      ...
      <div className="bg-purple-700 text-white rounded-xl p-5 shadow-lg mx-4">
        ...
        <div className="mt-4">
          ...
          <div className="text-sm text-purple-200 mt-1">
            ...
            {showBalance
              ? formatCurrency(userData?.weeklyRewards ?? 0)
              : "••••••••"}
          </div>
          {/* New Current Level display */}
          <div className="text-sm text-purple-200 mt-1">
