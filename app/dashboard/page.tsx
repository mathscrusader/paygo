...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // 2. Weekly referral rewards balance
      let weeklyRewards = 0
      const { data: profileData } = await supabase
        ...
        .maybeSingle()
      if (profileData?.reward_balance != null) {
        weeklyRewards = profileData.reward_balance
      }
      ...
      // 5. Upgrade level lookup
      const { data: upgradeProfileData } = await supabase
        .from("profiles")
        .select("upgrade_level_id")
        .eq("id", session.user.id)
        .maybeSingle()
      if (upgradeProfileData?.upgrade_level_id) {
        const { data: levelData } = await supabase
          .from("UpgradeLevel")
          .select("id, name")
          .eq("id", upgradeProfileData.upgrade_level_id)
          .maybeSingle()
        if (levelData) setCurrentLevel(levelData)
      }

      // 6. Unseen transactions count
  ...
  return (
    <div className="min-h-screen pb-4 bg-gradient-to-b from-[#f9f2f2] to-[#f5f0ff]">
      ...
      <div className="bg-purple-700 text-white rounded-xl p-5 shadow-lg mx-4">
          ...
          <div className="flex items-center gap-2">
            <div className="w-11 h-11 rounded-full bg-white flex items-center justify-center shadow-md overflow-hidden">
              {userData.profilePicture ? (
                <img src={userData.profilePicture} className="w-full h-full object-cover" alt={userData.name} />
              ) : (
        ...
        {/* Balances & Links (tighter spacing) */}
        <div className="mt-3">
          ...
          <Link href="/rewards" title="View details">
            <div className="text-sm text-white mb-1 flex items-center gap-1 hover:text-gray-200 cursor-pointer">
          ...
          <div className="flex justify-between items-center">
            <Link href="/upgrade" className="flex-1 mr-2">
              <Button className="w-full bg-purple-600 hover:bg-purple-500 text-white rounded-full py-3 flex items-center justify-center gap-2">
            ...
            <Link key={i} href={item.link || "#"}>
              <div className="flex flex-col items-center justify-center p-1 hover:-translate-y-1 transition-transform duration-300">
                <div className={`w-10 h-10 flex items-center justify-center mb-1 ${item.color} animate-pulse-slow rounded-lg`}>
                  {item.emoji ? <span className="text-2xl">{item.emoji}</span> : Icon && <Icon size={22} strokeWidth={1.5} />}
                </div>
