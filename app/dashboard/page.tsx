  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      if (!localStorage.getItem(welcomeKey)) {
        ...
        if (
          localStorage.getItem("paygo-new-month-popup-last-shown") !== monthKey
        ) {
      ...
      <div className="bg-white text-green-600 text-xs py-3 px-4 shadow-sm w-full">
        <div className="overflow-hidden whitespace-nowrap">
          <div className="animate-marquee inline-block font-bold">
            ...
            payments and get verified instantly!
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center gap-2">
            <Link href="/notifications">
              <Button
                variant="ghost"
                size="icon"
                className="relative h-10 w-10 rounded-full bg-green-600 hover:bg-green-500"
              >
        ...
        <div className="mt-4">
          ...
          <Link href="/rewards" title="View details">
            <div className="text-sm text-purple-200 mt-1 flex items-center gap-1 hover:text-white cursor-pointer transition-all group">
              ...
              <span>
                {showBalance
                  ? formatCurrency(userData.weeklyRewards)
                  : "••••••••"}
              </span>
          ...
          return (
            <Link key={index} href={item.link || "#"}>
              <div className="flex flex-col items-center justify-center p-1 hover:-translate-y-1 transition-transform duration-300">
                <div
                  className={`w-10 h-10 flex items-center justify-center mb-1 ${item.color} animate-pulse-slow rounded-lg`}
                >
                  {item.emoji ? (
